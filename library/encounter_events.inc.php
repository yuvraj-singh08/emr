...
function InsertEvent($args, $from = 'general')
{
    ...
    if ($from == 'general') {
        ...
        // Call Zoom API after successful event creation
        $zoom_meeting_payload = [
            'agenda' => $args['form_title'],
            'default_password' => false,
            'duration' => $args['duration'] / 60, // Convert duration to minutes
            'password' => (string)$pc_eid, // Use pc_eid as the password
            'pre_schedule' => true,
            'schedule_for' => 'yuvrajsingh08cs@gmail.com',
            'start_time' => date("Y-m-d\TH:i:s", strtotime($args['event_date'] . ' ' . $args['starttime'])) . '+05:30', // Use explicit timezone offset for IST
            'timezone' => 'Asia/Kolkata', // Keep the timezone as Asia/Kolkata
            'type' => 2
        ];
        

        $zoomApiResponse = callZoomApi($zoom_meeting_payload);

        if ($zoomApiResponse === false) {
            // Handle API error if needed
            error_log("Failed to create Zoom meeting for event ID: $pc_eid");
        }

        $GLOBALS['temporary-eid-for-manage-tracker'] = $pc_eid; // Used by manage tracker module
...
// Helper function to call the Zoom API
function callZoomApi($data)
{
    $url = 'http://localhost:8000/meeting/create';
    $options = [
        'http' => [
